...
export let inflightCount: IObservableValue<number> = observable(0);

const incInflightCount = action(() => {
    inflightCount.set(inflightCount.get() + 1);
});

const decInflightCount = action(() => {
    inflightCount.set(inflightCount.get() - 1);
});
